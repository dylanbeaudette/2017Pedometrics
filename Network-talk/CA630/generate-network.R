...
leg <- unique(data.frame(cluster=V(g)$cluster, color=V(g)$color, stringsAsFactors = FALSE))
leg <- leg[order(leg$cluster), ]
legend('bottomleft', legend=leg$cluster, col=leg$color, pch=15, ncol = 4)
dev.off()
